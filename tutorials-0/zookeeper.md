# 实战项目0：利用Docker搭建ZooKeeper集群

大家好！“搭建完全分布式的大数据集群”第一关要完成的实战项目是利用Docker搭建ZooKeeper集群。作为实验指导，本文首先会对ZooKeeper及其应用场景进行一个简单的介绍；然后手把手教大家构建基础操作系统镜像，这将会是后面很多工作的起点，在这个过程中也会给大家介绍一点编写Dockerfile的技巧；紧接着，我们将趁热打铁，使用刚学会的技巧来构建ZK镜像，在构建镜像的过程中会介绍与ZK有关的配置选项；最后，通过学习编排文件的编写技巧，我们将掌握在本地启动ZK集群的技能。
